...
CMSIS ?= ./CMSIS_5/CMSIS_5-5.9.0/
# Use QEMU_PATH= if QEMU was installed through the apt-get command.
QEMU_PATH ?= /usr/bin/
TOOLCHAIN ?= /opt/gcc-arm-none-eabi/gcc-arm-none-eabi-10.3-2021.10/bin/
...
3_8_1: $(SRC_C) $(3_8_1_SRC) 3_8_1_boot.o
	$(CC) $^ $(CFLAGS)  $(LDFLAGS) -T $(LINKER_SCRIPT) -o $(BINARY)
	$(OBJ) -D  -S $(BINARY) > $@_$(BINARY_OBJDUMP)
